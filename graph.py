...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        disjoint = Graph(False)
        vertices = {}   # dictionary used to map vertices in old graphs (keys) to the vertices in the new graph (values)

        # Adding vertices
        for vertex in self.vertices:
            disjoint_vertex = Vertex(disjoint)
            disjoint.add_vertex(disjoint_vertex)
            vertices[vertex] = disjoint_vertex
        ...
        for edge in self.edges:
            disjoint_edge = Edge(vertices[edge.tail], vertices[edge.head])
            disjoint += disjoint_edge
